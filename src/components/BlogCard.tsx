...
const BlogCard: React.FC<BlogCardProps> = ({ post }) => {
  ...
  return (
    <Link href={`/blog/${post.slug}`} className="block h-full" onClick={() => trackEvent('click', 'Blog Post', post.title)}>
      <motion.div
        className="group relative bg-[#1a1a1a] rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 h-full flex flex-col"
      ...
      >
        ...
        <div className="relative bg-[#1a1a1a] rounded-2xl p-4 md:p-6 m-1 flex-grow flex flex-col">
          ...
          {/* Meta Information */}
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between text-xs md:text-sm text-gray-400 mb-3 md:mb-4 space-y-2 sm:space-y-0 mt-auto">
